...
import { ValidatorConfig, loadConfig, ensureDirectories } from './config';
import { cloneRepoByUrl, scanSingleSkill, ScannedSkill } from './scanner';
import { deploySkill, cleanupWorkDir } from './deployer';
import { validateSkill, generateReport } from './validator';

/**
 * 主函数 - 支持单个 GitHub URL 测评
 */
export async function main(githubUrl?: string): Promise<void> {
  // 如果没有提供 URL，打印用法
  if (!githubUrl) {
  ...
  if (isLocal) {
    ...
    const localPath = await cloneRepoByUrl(repoUrl, config.storage.cacheDir);
  ...
  const skill = await scanSingleSkill(skillPath, sourceRepo);
  if (!skill) {
  ...
  const deployResult = await deploySkill(skill, config);
  ...
  const validateResult = await validateSkill(skill, workDir, config);
  console.log(`[Step 6] 测评完成, 状态: ${validateResult.status}`);

  // 7. 生成报告
  console.log('[Step 7] 生成测评报告...');
  const report = await generateReport(skill, {
 ...
 * 解析 GitHub URL 或本地路径
 */
function parseGitHubUrl(url: string): { owner: string; repo: string; skillName: string; isLocal: boolean; localPath: string } {
