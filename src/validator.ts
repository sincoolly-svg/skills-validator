...
export async function validateSkill(
  ...
  // 保存验证日志
  await saveLog(logDir, `${skill.name}_validate.log`, 
    `=== Skill 功能验证 ===\n` +
    `Skill: ${skill.name}\n` +
 ...
 * 读取 skill 的文档
 */
async function readSkillDocumentation(skillPath: string): Promise<{
  readme: string;
  hasReadme: boolean;
}> {
...
async function analyzeSkill(
  skill: ScannedSkill,
  docs: { readme: string; hasReadme: boolean }
): Promise<{
  type: 'shell' | 'node' | 'python' | 'api' | 'unknown';
  description: string;
  capabilities: string[];
  testStrategy: string;
  hasDocs: boolean;
  ...
  // 分析 skill 类型
  let type: 'shell' | 'node' | 'python' | 'api' | 'unknown' = 'unknown';
...
async function runFunctionalValidation(
  ...
  if (validate?.command && validate.type !== 'none') {
    ...
    const result = await execWithTimeout(validate.command, {
      cwd: workingDir,
      env: process.env,
      timeout
    });
 ...
 */
async function performDocumentationBasedTest(
  skill: ScannedSkill,
  analysis: {
    type: 'shell' | 'node' | 'python' | 'api' | 'unknown';
    description: string;
  ...
  try {
    ...
    for (const bin of requiredBins) {
      ...
      try {
        const result = await execWithTimeout(checkCmd, { timeout: 5000 });
    ...
    if (missingBins.length > 0 && installCommands.length > 0) {
      ...
      for (const installCmd of installCommands) {
        ...
        try {
          const result = await execWithTimeout(installCmd, { 
    ...
    if (usageExamples.length > 0) {
      ...
      try {
        const result = await execWithTimeout(testCmd, {
          cwd: skillPath,
          env: process.env,
          timeout: 60000 // 1分钟超时
        });
        ...
        if (result.exitCode !== 0) {
          ...
          try {
            const helpResult = await execWithTimeout(helpCmd, {
              cwd: skillPath,
              env: process.env,
              timeout: 10000
            });
    ...
    const entryFile = files.find(f => 
      f === 'index.js' || f === 'main.js' || f === 'skill.js' || 
 ...
 */
async function performFunctionalTest(
  skill: ScannedSkill,
  analysis: {
    type: 'shell' | 'node' | 'python' | 'api' | 'unknown';
    description: string;
 ...
 */
function generateTestInput(skillName: string, analysis: { description: string; capabilities: string[] }): string {
 ...
 * 获取建议
 */
function getRecommendation(status: ValidationReport['status']): string {
  switch (status) {
    case 'PASSED':
      return '生产可用';
    case 'FAILED':
